...
- 优化 TileService，移除不必要的导入和启动 Activity 的逻辑，改为直接处理开关逻辑以提升用户体验

- 优化消息发送内容展示，限制文本行数为8行

## v0.8.89-pre.3

- 优化通知渠道设置，禁用状态栏图标显示并隐藏锁屏显示

- 优化消息发送逻辑，添加消息长度限制并打印发送内容

- 更新 ICHANGELOG.md 文件并修正 Telegram 推送中的聊天 ID 和链接
...
- Merge pull request #5 from wolfsilver/copilot/fix-4

- [WIP] ✨ Set up Copilot instructions
- Update .gitignore
- Change submodule URLs to use SSH format
- Clean up accidentally committed build artifacts and update .gitignore

- Co-authored-by: wolfsilver <2452450+wolfsilver@users.noreply.github.com>

- Add comprehensive GitHub Copilot instructions for FlClash development
